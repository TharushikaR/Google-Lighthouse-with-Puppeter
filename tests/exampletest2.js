...
(async () => {
    ...
    try {
        equal(scoresBelowBaseline, false, 'One of the scores was found below baseline. Failing test');
    } catch (error) {
...
async function runLighthouseForURL(pageURL, opts, reportName) {
    ...
    const html = ReportGenerator(report.lhr, 'html');
    const json = ReportGenerator(report.lhr, 'json');
    ...
    writeFile('ReportHTML-' + reportNameForFile + '.html', html, (err) => {
        if (err) {
            console.error(err);
        }
    });

    writeFile('ReportJSON-' + reportNameForFile + '.json', json, (err) => {
        if (err) {
            console.error(err);
        }
    });

    writeFile('ReportScores-' + reportNameForFile + '.txt', JSON.stringify(scores, null, 2), (err) => {
        if (err) {
            console.error(err);
        }
    });
